...
let teileTexte = (text) => {
    let myArr = []
    let maxPart = Math.ceil(text.length / 100)
    console.log(maxPart);
    for (let i = 0; i < maxPart; i++) {
        // debugger;
        let start = i * 100;
        let satz = text.substring(start, start + 100)
        console.log(satz);
        myArr.push(`${satz} - ${i + 1} of ${maxPart}`)
    }
    return myArr
}


console.log(teileTexte(someText));
